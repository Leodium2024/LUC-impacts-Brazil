...
########################################################################################
########################################################################################
################      Functions to import Absence/Presence data     ####################
########################################################################################
########################################################################################
...
Get_LU_year <- function (data) {
  ...
  for (year in 1990:2022) {
    ...
    data$Grassland <- ifelse(data$year == year, data[[paste0("Grass", year)]] + data[[paste0("Past", year)]], data$Grassland)
  ...
  cols_to_check <- c('Forest', 'Grassland', 'Agriculture', 'Forestry',"Bio_1","Bio_4", "Bio_12","Bio_15", "Elevation", "Slope")
...
Get_Mean <- function(data, ott_id){
  
  # Filter the data where Presence_absence is 1
  data <- data[data$Presence_Absence == 0, ]
  
  mean_values <- sapply(data[, c('Forest', 'Grassland', 'Agriculture', 'Forestry',"Bio_1","Bio_4", "Bio_12","Bio_15", "Elevation", "Slope")], mean, na.rm = TRUE)
  mean_df <- data.frame(t(mean_values))
  mean_df$ott_id <- ott_id
  mean_df <- mean_df[, c('ott_id', colnames(mean_df)[-which(colnames(mean_df) == 'ott_id')])]
...
Extract_coefficients <- function(model, Variable_names, ott_id) {
  ...
  if (Forest == 0  && Grassland == 0 && Agriculture == 0 &&
  ...
  new_row <- c(ott_id, Forest, Grassland, Agriculture, Forestry, 
...
Species_mean<- data.frame(matrix(ncol = 11, nrow = 0))
#Predictors
Predictors_All<- c('Forest', 'Grassland', 'Agriculture', 'Forestry',"Bio_1","Bio_4", "Bio_12","Bio_15", "Elevation", "Slope")
...
for (i in 1:num_Species) { 
  ...
  Species_mean<- rbind(Species_mean, Get_Mean(Abs_Pres,Ott_id))
...
Output_directory <- "********/Input/Biodiversity"
...
write_xlsx(Coef_df_AIC, file.path(Output_directory, "Species_coef.xlsx"))
write_xlsx(Variable_importance_df, file.path(Output_directory, "Variable_importance.xlsx"))
write_xlsx(Performance_AIC, file.path(Output_directory, "Performance.xlsx"))
write_xlsx(Calibrating_data, file.path(Output_directory, "Calibrating.xlsx"))
